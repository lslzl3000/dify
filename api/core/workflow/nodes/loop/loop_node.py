...
class LoopNode(Node):
    ...
    def _run(self) -> Generator:
        ...
        outputs: dict[str, Any] = {}  # Final outputs from the loop
        ...
        try:
            ...
            for i in range(loop_count):
                ...
                reach_break_condition = yield from self._run_single_loop(
        ...
        *,
        ...
        for event in graph_engine.run():
            ...
            if isinstance(event, NodeRunSucceededEvent) and event.node_type == NodeType.LOOP_END:
                reach_break_condition = True

            if isinstance(event, GraphRunFailedEvent):
                # Handle graph run failure
                raise Exception(event.error)
            elif isinstance(event, GraphNodeEventBase):
                self._append_loop_info_to_event(event=event, loop_run_index=current_index)
                yield event

            # After yielding a succeeded event, check break conditions
        ...
        for loop_var in self._node_data.loop_variables or []:
            ...
            outputs[key] = segment.value if segment else None
        outputs["loop_round"] = current_index + 1
        self._node_data.outputs = outputs
        if reach_break_condition:
            return reach_break_condition

        yield LoopNextEvent(
            index=current_index + 1,
            pre_loop_output=outputs,
        )
        return False
